...
public class GoogleCloudStorageService : IStorageService
{
    ...
    public GoogleCloudStorageService(DiplomaMakingContext context, IConfiguration configuration, FileUtilityService fileUtilityService, IWebHostEnvironment env)
    {
        ...
        _bucketName = _env.IsDevelopment() || _env.IsEnvironment("Test")
            ? configuration["GoogleCloud:BucketName"]
                ?? throw new ArgumentNullException("GoogleCloud:BucketName configuration is missing")
            : Environment.GetEnvironmentVariable("BucketName")
                ?? throw new ArgumentNullException("BucketName environment variable is missing");
    ...
    public async Task<byte[]?> GetFileFromPath(string templateName, string subDirectory = "DiplomaPdfs")
    {
